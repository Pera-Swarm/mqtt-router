    ...
    start = () => {
        ...
        this.mqttClient.on('message', (topic, message, packet) => {
            ...
            for (var i = 0; i < this.routes.length; i += 1) {
                if (channel + this.routes[i].topic === topic) {
                    var msg;
    ...
    handleRouteSubscriptions = () => {
        for (var i = 0; i < this.routes.length; i++) {
            if (this.routes[i].subscribe != false) {
                ...
                console.log('MQTT_Subscribed: ', channel + this.routes[i].topic);

                this.mqttClient.subscribe(channel + this.routes[i].topic, this.options);
            } else {
                // No subscription required for this topic
                console.log('MQTT_Not_Subscribed: ', channel + this.routes[i].topic);
            }
        }
        console.log('');
    };
